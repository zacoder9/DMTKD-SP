...
def train(args):
    ...
    for _ in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_loader):
            ...
            # 只使用静态权重
            loss = loss_ce.mean() + args.kd_weight * rep_loss + args.kd_weight * transfer_sparql_loss_ce.mean() + args.kd_weight * transfer_dcs_loss_ce.mean()
...
def main():
    ...
    parser.add_argument('--output_dir', default='./output/without_confidence/checkpoint')

    parser.add_argument('--save_dir', default='./output/without_confidence/log_raw', help='path to save checkpoints and logs')
