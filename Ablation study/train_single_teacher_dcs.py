...
#     parser.add_argument('--model_name_or_path', default='./bart-kopl')
...
def train(args):
    ...
    for _ in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_loader):
            ...
            outputs_program = [tokenizer.decode(output_id, skip_special_tokens=True, clean_up_tokenization_spaces=True)
                               for output_id in outputs_p]
            ...
            new_input_ids = tokenizer.batch_encode_plus(new_questions, max_length=512, pad_to_max_length=True,
                                                        truncation=True)
            ...
            outputs_kopl = [tokenizer.decode(output_id, skip_special_tokens=True, clean_up_tokenization_spaces=True) for
                            output_id in y]
            outputs_kopl = [post_process(output) for output in outputs_kopl]

            # transfer_sparql_outputs_kopl = [
            #     transfer_sparql_tokenizer.decode(output_id, skip_special_tokens=True, clean_up_tokenization_spaces=True)
            #     for output_id in sparql_2_kopl]
            # transfer_sparql_outputs_kopl = [post_process(output) for output in transfer_sparql_outputs_kopl]

            transfer_dcs_outputs_kopl = [
                transfer_dcs_tokenizer.decode(output_id, skip_special_tokens=True, clean_up_tokenization_spaces=True)
                for output_id in dcs_2_kopl]
            transfer_dcs_outputs_kopl = [post_process(output) for output in transfer_dcs_outputs_kopl]

            # sparql_kd_weight = len(set(transfer_sparql_outputs_kopl) & set(outputs_kopl)) / args.batch_size
            dcs_kd_weight = len(set(transfer_dcs_outputs_kopl) & set(outputs_kopl)) / args.batch_size
            ...
            loss = loss_ce.mean() + dcs_kd_weight * transfer_dcs_loss_ce.mean()
...
def main():
    parser = argparse.ArgumentParser()
    # input and output
    parser.add_argument('--input_dir', default='./dataset/plus_subset/preprocessed_data')
    parser.add_argument('--output_dir', default='./output/single_teacher_dcs/checkpoint')

    parser.add_argument('--save_dir', default='./output/single_teacher_dcs/log_raw', help='path to save checkpoints and logs')
    parser.add_argument('--model_name_or_path', default='./output/single_teacher_dcs/checkpoint/checkpoint-75175')
    ...
    # parser.add_argument("--teacher-ckpt-sparql", default='./teacher-ckpt-sparql/checkpoint')
    parser.add_argument("--teacher-ckpt-dcs", default='./teacher-ckpt-dcs/checkpoint')
    ...
    # parser.add_argument("--teacher-ckpt-kopl", default='./teacher-ckpt-kopl')  # 这个就是baseline(Bart)模型
    parser.add_argument("--kd_rep_weight", type=float, default=0.8)

    # transfer_based
    # parser.add_argument("--transfer-ckpt-sparql", default='./transfer-ckpt-sparql/checkpoint')
    parser.add_argument("--transfer-ckpt-dcs", default='./transfer-ckpt-sparql/checkpoint')
